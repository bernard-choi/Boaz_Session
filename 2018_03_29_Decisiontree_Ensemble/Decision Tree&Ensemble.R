...
#tree 패키지는 binary recursive partitioning을,
#rpart 패키지는 CART(classification and regression trees) 방법론을 사용합니다.
#문제점 : 두개의 패키지는 엔트로피, 지니계수를 기준으로 가지치기를 할 변수를 결정하기 때문에 상대적으로 연산 속도는 빠르지만 과적합화의 위험성이 존재합니다.
#해결방안 : 두 패키지를 사용할 경우에는 Pruning 과정을 거쳐서 의사결정나무를 최적화 하는 과정이 필요합니다.

# party 패키지는 Unbiased recursive partitioning based on permutation tests 방법론을 사용합니다.
# 장점 : p-test를 거친 Significance를 기준으로 가지치기를 할 변수를 결정하기 때문에 biased 될 위험이 없어 별도로 Pruning할 필요가 없다.
# 문제점 : 대신 입력 변수의 레벨이 31개 까지로 제한되어 있다.

####################################################################################
##### 의사결정나무 분석 방법 #######################################################
####################################################################################

# 일반적으로 데이터 준비 – 의사결정나무만들기 – 가지치기 – 예측 및 모델 평가의 단계로 진행된다.
...
# 해석 방법 - 참고
# 1) n=105은 150개의 데이터가 있음을 의미.
# 2)들여쓰기는 가지의 갈라지는 모양을 뜻한다.
# 3) '*'는 잎사귀 노드를 의미한다.
# 4) 트리의 최상단은 root노드로 위의 결과에는 '1)'로 표시된다. 괄호안은 iris의 Species별 비율을 의미한다.
...
#tree패키지에서 제공하는 cv.tree 함수를 사용하면 간단하게 확인할 수 있다.

cv.trees<-cv.tree(treemod, FUN=prune.misclass )  # misclass : 교차검증 오분류율 기준
plot(cv.trees)

#교차검증 옵션의 디폴트는 deviance이다.
...
#rpart.control에서 cp값을 0으로 설정하면 나무모형의 오분류값이 최소가 될 때 까지 분할을 진행한다. 
...
# 데이터 셋이나 변수의 특징에 따라 사용할 패키지를 선택하면 된다.
